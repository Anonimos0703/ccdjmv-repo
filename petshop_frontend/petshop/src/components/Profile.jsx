...
const Profile = ({ onProfileImageUpdate }) => {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/auth/users/${id}/upload-profile-pic`, formData, {
        headers: {
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        <Grid container spacing={4}>
          <Grid item xs={12} md={4} sx={{ textAlign: "center" }}>
            ...
            <Button variant="contained" onClick={handleImageUpload} startIcon={<CloudUploadIcon />} sx={{ mt: 2 }}>
              Upload Picture
            </Button>
          </Grid>
