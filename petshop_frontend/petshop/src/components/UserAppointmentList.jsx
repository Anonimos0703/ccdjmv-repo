...
const UserAppointmentList = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/appointments/byUserEmail/${email}`);
        if (!response.ok) {
    ...
    try {
      ...
      setAppointments((prevAppointments) =>
        prevAppointments.map((appointment) =>
          appointment.appId === appId ? { ...appointment, canceled: true } : appointment
        )
      );
  ...
  const handleDelete = async (appId) => {
    try {
      const response = await fetch(`http://localhost:8080/api/appointments/deleteAppointment/${appId}`, {
        method: "DELETE",
      });
      ...
      setAppointments((prevAppointments) => prevAppointments.filter((appointment) => appointment.appId !== appId));
